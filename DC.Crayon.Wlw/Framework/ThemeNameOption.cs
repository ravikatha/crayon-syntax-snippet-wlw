...
{
	[Serializable]
	internal class ThemeNameOption : CustomOptionListOption
	{
		#region Overrides
		/// <summary>
		...
		public override Control CreateEditorControl(Control parentControl, object value, IProperties pluginProperties)
		{
			...
			FillItems(comboBox, (value ?? string.Empty).ToString().Trim(), true,
				parentControl.Controls.OfType<TextBox>().SingleOrDefault(c => c.Tag == Options.CustomThemesOption),
				Options.StandardThemeListItems, Options.CustomThemesOption, pluginProperties);
		...
		public override void SetValueToControl(Control control, object value, IProperties pluginProperties)
		{
			...
			FillItems(comboBox, (value ?? string.Empty).ToString().Trim(), true,
				control.Parent.Controls.OfType<TextBox>().SingleOrDefault(c => c.Tag == Options.CustomThemesOption),
				Options.StandardThemeListItems, Options.CustomThemesOption, pluginProperties);
		}
		#endregion
	}
